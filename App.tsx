...
import { Download, RefreshCw, Wand2, ArrowLeft, AlertCircle, Shirt, User, Briefcase, Minus, LayoutGrid, Image as ImageIcon, Lock, Mail, Eye, EyeOff, LogOut, Key } from 'lucide-react';
...
const App = () => {
  ...
  const handleProcess = async () => {
    ...
    if (needsAI && !hasSelectedKey) {
      const confirmed = window.confirm("AI enhancement requires a Gemini API key. Would you like to select one now? (You can use a free key, but paid keys have higher limits)");
      if (confirmed) {
    ...
    try {
      ...
      setState(AppState.PREVIEW);
        ...
        if (errorMessage.includes("429") || errorMessage.includes("quota")) {
          setError("API Quota exceeded. Please select a paid API key or wait for the quota to reset.");
          setIsProcessing(false);
          return;
        }
        ...
        <div className="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button 
              ...
              className={`text-sm font-bold flex items-center gap-1.5 transition-all px-3 py-1.5 rounded-lg ${hasSelectedKey ? 'text-green-600 bg-green-50 hover:bg-green-100' : 'text-amber-600 bg-amber-50 hover:bg-amber-100 animate-pulse'}`}
              title={hasSelectedKey ? "API Key Selected" : "API Key Required for AI"}
            >
              <Key size={14} /> {hasSelectedKey ? "API Key Active" : "Setup API Key"}
            </button>
